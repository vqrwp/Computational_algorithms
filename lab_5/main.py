...
def gauss(left, right):
    ...
    for i in range(0, n):
        for j in range(i + 1, n):
            sep = left[j][i] / left[i][i]
            for k in range(0, n):
                left[j][k] -= left[i][k] * sep
            right[j] -= right[i] * sep

    for i in range(n - 1, -1, -1):
        for k in range(i + 1, n):
            right[i] -= left[i][k] * right[k]
        right[i] /= left[i][i]

    return right


# middle method of integrating
...
def find_K(T, d_e):
    ...
    for i in range(4):
        Q_ip1 = interpolate(T, 4, Q_data[0], Q_data[i + 2])
        Q_i = interpolate(T, 4, Q_data[0], Q_data[i + 1])
...
def check_gamma(gamma, T, X):
    right_part = exp(X[0]) / (1 + gamma / 2)

    for i in range(1, 6):
        right_part += ((exp(X[i]) * Z_c[i - 1] * Z_c[i - 1]) / (1 + Z_c[i - 1] * Z_c[i - 1] * gamma / 2))
...
def find_gamma(st, end, T, X):
    while abs(st - end) > eps:
        ...
        if check_gamma(cur_gamma, T, X) <= 0:
            st = cur_gamma
        else:
            end = cur_gamma
...
def find_nt(T, P, X):
    ...
    while True:
        ...
        left = [[1, -1, 1, 0, 0, 0],
                ...
                [exp(X[0]), 0, -Z_c[1] * exp(X[2]), -Z_c[2] * exp(X[3]), -Z_c[3] * exp(X[4]),
                 -Z_c[4] * exp(X[5])]]

        alpha = 0.285 * pow(10, -11) * pow(gamma * T, 3)

        right = [log(K[0]) + X[1] - X[2] - X[0],
                 log(K[1]) + X[2] - X[3] - X[0],
                 log(K[2]) + X[3] - X[4] - X[0],
                 log(K[3]) + X[4] - X[5] - X[0],
                 exp(X[0]) + exp(X[1]) + exp(X[2]) + exp(X[3]) + exp(X[4]) + exp(X[5]) - alpha -
                 calc_nt(T, P),
                 Z_c[1] * exp(X[2]) + Z_c[2] * exp(X[3]) + Z_c[3] * exp(X[4]) + Z_c[4] *
                 exp(X[5]) - exp(X[0])]

        d_X = gauss(left, right)
        # d_X = solve(left, right) # ~30% быстрее
...
def main():
    ...
    P0 = float(input("P0: "))  # start pressure
    T0 = float(input("T0: "))  # start temperature
    Tw = float(input("Tw: "))  # end temperature
    m = float(input("m: "))  # mass

    start = time()

    while abs(P_max - P_min) > eps:
        # method of the dichotomy
        curr_p = abs(P_max + P_min) / 2.0
        curr_p += P_min

        # integrate from 0 to 1, z - iteration
        integral_value = integrate(0, 1, lambda z: find_nt(t(z, T0, Tw, m), curr_p, X) * z)

        # 293 K - base temperature, it is integration, checking accuracy
        if (calc_nt(293, P0) - 2 * integral_value) >= 0:
            P_min = curr_p
        else:
            P_max = curr_p
